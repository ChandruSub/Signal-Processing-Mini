...
st.title("Low-Pass and High Pass Filter")

#the low-pass filter
def butter_lowpass(cutoff, fs, order=5):
    nyquist = 0.5 * fs #nyquist rate from signals and systems
...
def main():
    ...
    if uploaded_file is not None:
        ...
        filter_type = st.radio("***Select the Filter type :***", ["Lowpass", "Highpass"], index=0)
        ...
        if filter_type == "Lowpass":
            ...
            st.header("High-Pass Filter Controls")
            st.write("Filter Settings:")
            cutoff = st.slider("Cutoff Frequency (Hz) :", 1000, 5000, 2000)
            order = st.slider("Filter Order :", 1, 8, 3)

            # Apply high-pass filter
            filtered_data = apply_highpass_filter(data, cutoff, fs, order)

            # Normalize the filtered data
            filtered_data = np.int16(filtered_data / np.max(np.abs(filtered_data)) * 32767)

        else:
            st.write("Please select a filter type to apply it.")
